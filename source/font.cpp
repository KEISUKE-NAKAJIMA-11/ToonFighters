...
Font::Font(ID3D11Device* device, const char* filename, int maxSpriteCount)
{
	...
	// 頂点シェーダー
	{
		...
		create_vs_from_cso(device, "./Shader/font_vs.cso", vertexShader.GetAddressOf(), inputLayout.GetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	}

	// ピクセルシェーダー
	{
		create_ps_from_cso(device, "./Shader/font_ps.cso", pixelShader.GetAddressOf());
	}
...
void Font::Draw(float x, float y, const wchar_t* string,DirectX::XMFLOAT4 color, DirectX::XMFLOAT2 scale)
{
	...
	for (size_t i = 0; i < length; ++i)
	{
		...
		currentVertex[1].position.x = positionX + (info.width*scale.x);
		...
		currentVertex[2].position.y = positionY + (info.height*scale.y);
		...
		currentVertex[3].position.x = positionX + (info.width * scale.x);
		currentVertex[3].position.y = positionY + (info.height * scale.y);
