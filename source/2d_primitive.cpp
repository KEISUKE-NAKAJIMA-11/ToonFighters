    ...
    vertexShader(nullptr), pixelShader(nullptr), inputLayout(nullptr), buffer(nullptr), rasterizerState(nullptr)
{
    ...
    //	頂点シェーダーの読み込み
    if (!resourceManager::LoadVertexShader(device, "./Shader/2d_primitive_vs.cso", layout, numElements, &vertexShader, &inputLayout))
    {
        assert(!"頂点シェーダーの読み込みに失敗(Primitive)");
        return;
    }
    //	ピクセルシェーダーの作成
    if (!resourceManager::LoadPixelShader(device, "./Shader/2d_primitive_ps.cso", &pixelShader))
    {
...
PrimitiveBatch::PrimitiveBatch(ID3D11Device* device, size_t maxInstance)
{
    ...
    UINT numElements = ARRAYSIZE(layout);
    if (!resourceManager::LoadVertexShader(device, "./Shader/2d_primitive_batch_vs.cso", layout, numElements, &vertexShader, &inputLayout))
    {
        assert(!"頂点シェーダーの読み込みに失敗(PrimitiveBatch)");
        return;
    }
    if (!resourceManager::LoadPixelShader(device, "./Shader/2d_primitive_batch_ps.cso", &pixelShader))
    {
...
namespace primitive
{
        ...
        bool world)
    {
        ...
        if (world)
        {
            float scaleFactor = view_set2d::getScale();
            VECTOR2 s = size * scaleFactor;
            VECTOR2 c = center * scaleFactor;
            VECTOR2 pos = view_set2d::worldToScreen(position);
            pSystem.primitive_2d->rect(pSystem.devicecontext.Get(), pos, s, c, angle, color);
        }
        else
        {
            pSystem.primitive_2d->rect(pSystem.devicecontext.Get(), position, size, center, angle, color);
        }
    }
        ...
        bool world)
    {
        if (world)
        {
            ...
            VECTOR2 v1 = view_set2d::worldToScreen(from);
            VECTOR2 v2 = view_set2d::worldToScreen(to);
            pSystem.primitive_2d->line(pSystem.devicecontext.Get(), v1, v2, color, w);
        }
        else
        {
            pSystem.primitive_2d->line(pSystem.devicecontext.Get(), from, to, color, width);
        }
    }
        ...
        bool world)
    {
        ...
        {
            float scaleFactor = view_set2d::getScale();
            VECTOR2 v1 = view_set2d::worldToScreen(pos);
            pSystem.primitive_2d->circle(pSystem.devicecontext.Get(), v1, radius * scaleFactor, color, n);
        }
        else
        {

            pSystem.primitive_2d->circle(pSystem.devicecontext.Get(), pos, radius, color, n);
        }
    }
        ...
        bool world)
    {
        ...
        {
            VECTOR2 q[4] = {};
            for (int i = 0; i < 4; i++) q[i] = view_set2d::worldToScreen(v[i]);
            pSystem.primitive_2d->quad(pSystem.devicecontext.Get(), q, color);
        }
        else
        {
            pSystem.primitive_2d->quad(pSystem.devicecontext.Get(), v, color);
        }
    }
    ...
    void draw_triangle_fan(VECTOR2 vertex[], const VECTOR4& color, int nPolygon, bool world)
    {
        if (world)
        {
            for (int i = 0; i < nPolygon + 2; i++) vertex[i] = view_set2d::worldToScreen(vertex[i]);
            pSystem.primitive_2d->draw_triangle_fan(pSystem.devicecontext.Get(), vertex, color, nPolygon);
        }
        else
        {
...
namespace primitive_batch
{
    ...
    void begin()
    {
        pSystem.primitive_batch_2d->begin(pSystem.devicecontext.Get());
    }
    ...
    void end()
    {
        pSystem.primitive_batch_2d->end(pSystem.devicecontext.Get());
    }
        ...
        bool world)
    {
        if (world)
        {
            float scaleFactor = view_set2d::getScale();
            VECTOR2 s = size * scaleFactor;
            VECTOR2 v = view_set2d::worldToScreen(pos);
            pSystem.primitive_batch_2d->rect(v, s, center, angle, color);
        }
        else
        {

            pSystem.primitive_batch_2d->rect(pos, size, center, angle, color);
        }
    }
        ...
        bool world)
    {
        ...
        if (world)
        {
            ...
            VECTOR2 v1 = view_set2d::worldToScreen(from);
            VECTOR2 v2 = view_set2d::worldToScreen(to);
            pSystem.primitive_batch_2d->line(v1, v2, color, w);
        }
        else
        {

            pSystem.primitive_batch_2d->line(from, to, color, width);
        }
    }
