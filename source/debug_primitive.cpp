...
DebugRenderer::DebugRenderer(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		...
		create_vs_from_cso(device, "./Shader/debug_vs.cso", vertexShader.GetAddressOf(), inputLayout.GetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	...
	// ピクセルシェーダー
	{
		...
		create_ps_from_cso(device, "./Shader/debug_ps.cso", pixelShader.GetAddressOf());
