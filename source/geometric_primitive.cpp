...
geometric_primitive::geometric_primitive(ID3D11Device* device)
{
	...
	create_vs_from_cso(device, "geometric_primitive_vs.cso", &vertex_shader, &input_layout, input_element_desc, ARRAYSIZE(input_element_desc));
	create_ps_from_cso(device, "geometric_primitive_ps.cso", &pixel_shader);
...
void geometric_primitive::render(ID3D11DeviceContext* immediate_context,
	...
	const DirectX::XMFLOAT4& material_color, bool wireframe)
{
	cbuffer data;
...
geometric_cylinder::geometric_cylinder(ID3D11Device* device, u_int slices) : geometric_primitive(device)
{
	...
	create_buffers(device, vertices.data(), (int)vertices.size(), indices.data(), (int)indices.size());
}

geometric_sphere::geometric_sphere(ID3D11Device* device, u_int slices, u_int stacks,float r) : geometric_primitive(device)
{
	std::vector<vertex> vertices;
	std::vector<u_int> indices;
...
geometric_board::geometric_board(ID3D11Device* device) : geometric_primitive(device)
{
	...
	create_buffers(device, vertices, ARRAYSIZE(vertices), indices, ARRAYSIZE(indices));
}

GeometricRect::GeometricRect(ID3D11Device* device) : geometric_primitive(device)
{
	vertex vertices[4] = {};
	unsigned int indices[3 * 2] = {};
